...
export default function UniversitiesPage() {
  ...
  const fetchRows = async (page = 1, activeFilters = filters) => {
    try {
      ...
      const items = response?.items || response?.universities || [];
      ...
      toast.error(err.response?.data?.message || "Failed to load university admins");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggle = async (account) => {
    if (account.isActive) {
      const shouldDisable = await confirm({
        title: "Disable University Admin",
        message: "This account will no longer be able to login until re-enabled.",
        confirmText: "Disable",
        cancelText: "Cancel",
        tone: "danger"
      });
      ...
      try {
        await disableUniversity(account._id, reason || "Disabled by Super Admin");
        toast.success("University admin disabled");
    ...
    const shouldEnable = await confirm({
      title: "Enable University Admin",
      message: "This account will regain platform access.",
      confirmText: "Enable",
      cancelText: "Cancel",
      tone: "primary"
    });
    ...
    try {
      await enableUniversity(account._id);
      toast.success("University admin enabled");
      await fetchRows(meta.page, filters);
    } catch (err) {
    ...
    <>
      <div className="space-y-5 sm:space-y-6">
        <header className="tx-page-header p-5 sm:p-6 md:p-8">
          <h1 className="text-2xl font-black text-slate-950 sm:text-3xl">University Admin Accounts</h1>
          <p className="mt-2 text-sm text-slate-500">
            Manage university placement admin accounts and monitor subscription/payment status.
          </p>
        </header>
          ...
          <form onSubmit={applyFilters} className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            <label className="text-sm text-slate-600">
              ...
              <input
                ...
                placeholder="University admin name or email"
        ...
        <section className="tx-card overflow-hidden">
          <div className="border-b border-slate-200 px-4 py-3 sm:px-5">
            <h2 className="text-lg font-bold text-slate-900">Universities</h2>
            <p className="text-xs text-slate-500">Total records: {meta.total.toLocaleString("en-IN")}</p>
          </div>

          {loading ? (
            <div className="p-5 text-sm text-slate-500">Loading university admins...</div>
          ) : rows.length === 0 ? (
            <div className="p-5 text-sm text-slate-500">No university admins found.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                  <tr>
                    <th className="px-4 py-3">University Admin</th>
                  ...
                  {rows.map((account) => (
                    <tr key={account._id} className="border-t border-slate-100 align-top">
                      ...
                      <td className="px-4 py-3 text-slate-700">
                        {account.totalStudents === null ? "N/A" : Number(account.totalStudents || 0).toLocaleString("en-IN")}
                      </td>
                      <td className="px-4 py-3">
